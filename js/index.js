var config = {NYT_KEY:"tMGUWfrj1uvYntT1Z6wFXOgzdJX2VNYP",GOOGLE_BOOKS_KEY:"AIzaSyCMJpNfT7RcbNJmlCFwfu5x0LlqynMAYHo"}

var nytimesKey = config.NYT_KEY;
var googleBooksKey = config.GOOGLE_BOOKS_KEY;

fetch('https://api.nytimes.com/svc/books/v3/lists.json?list-name=hardcover-fiction&api-key=tMGUWfrj1uvYntT1Z6wFXOgzdJX2VNYP', {
    method: 'get',
  })
...
function updateCover(id, isbn) {
  fetch('https://www.googleapis.com/books/v1/volumes?q=isbn:' + isbn + "&key=AIzaSyCMJpNfT7RcbNJmlCFwfu5x0LlqynMAYHo", {
    method: 'get'
  })
  .then(response => { return response.json(); })
  .then(data => {
